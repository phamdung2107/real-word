...
import { DEFAULT_REGISTER_USER } from "../../models/user";
...
function SignUp() {
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      await register(data);
      navigate("/login", {
        state: { message: "register success, please login to continue" },
      });
    } catch (error) {
      console.log(error);
  ...
  return (
    <Container size={1100} mt="1rem" style={{ height: "calc(100vh - 132px)" }}>
      <Flex direction="column" align="center">
        ...
        >
          <TextInput {...getInputProps("username")} />
          <TextInput {...getInputProps("email")} />
          <PasswordInput {...getInputProps("password")} />
          <Button
