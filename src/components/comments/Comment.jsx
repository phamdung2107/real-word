...
export default function Comment({ article, setArticle }) {
  const auth = useContext(AuthContext);
  const { postId } = useParams();
  const [postCommentLoading, setPostCommentLoading] = useState(false);
  const [newComment, setNewComment] = useState(DEFAULT_COMMENT_RESPONSE);
  ...
  const handleComment = async (formData) => {
    ...
    try {
      const newComment = await postComment(postId || "", formData.body);
